...
void SPI1_WriteReg(uint8_t address, uint8_t value) {//Viet 1 byte du lieu vao thanh ghi MFRC522, khong tra ve
	cs_reset();
	SPI1SendByte(address);//dia chi ghi vao dc dinh dang 0XXX XXX0
	SPI1SendByte(value);//gia tri ghi vao`
	cs_set();
}

uint8_t SPI1_ReadReg(uint8_t address) {//Đọc 1 byte từ thanh ghi MFRC522, trả về giá trị trong thanh ghi, tại địa chỉ đó.
	uint8_t	val;

	cs_reset();
	SPI1SendByte(address);//địa chỉ đc định dạng 1XXX XXX0
...
uint8_t MFRC522_Auth(uint8_t authMode, uint8_t BlockAddr, uint8_t* Sectorkey, uint8_t* serNum) {
	...
	// Verify the command block address + sector + password + card serial number//Xac nhan lenh + Khoi dia chi + mat khau + so nhanh
	buff[0] = authMode;
	buff[1] = BlockAddr;
